...
if page == "Overview":
    ...
    sample_cv = Path(__file__).resolve().parent / "sample_student_cv.pdf"
    if sample_cv.exists():
        with open(sample_cv, "rb") as f:
            st.download_button("📥 Download Sample Student CV (PDF)", data=f.read(), file_name="sample_student_cv.pdf", mime="application/pdf", help="Click to download a ready-made CV PDF to test the live parser.")
